...
pid = PIDController(kp=0.7, ki=0.01, kd=0.4, output_min=-60, output_max=60)
...
while True:
    ...
    ret1, quick_stop = cv2.threshold(quick_stop, 130, 255, cv2.THRESH_BINARY_INV)
    asd = np.mean(quick_stop)    #判断停止
    
    image1 = video_proc.Canny(image,60,80)    #边缘处理
    
    #只取中间
    cv_height = height//3
    image = image[cv_height:2*cv_height, :]
    #二值化处理
    ret2, image = cv2.threshold(image, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    
#     imshow_gray(image)      #显示黑白图
    
    image1 = image1[cv_height:2*cv_height, :]  #边缘处理的图像结果取中间
    image2 = image + image1                    #边缘加二值
    
    #分左，中，右三个区域
    part_width = width // 3
    
    left_part = image2[:, 0:part_width]
    middle_part = image2[:, part_width:part_width*2]
    right_part = image2[:, part_width*2:]
    
    imshow_gray(image2)    #显示处理后的图像
