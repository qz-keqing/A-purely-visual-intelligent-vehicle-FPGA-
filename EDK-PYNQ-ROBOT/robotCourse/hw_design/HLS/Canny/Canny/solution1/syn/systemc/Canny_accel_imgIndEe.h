...
#ifndef _Canny_accel_imgIndEe_H_
#define _Canny_accel_imgIndEe_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "Canny_accel_imgIndEe_memcore.h"

SC_MODULE(Canny_accel_imgIndEe) {
    static const unsigned int DataWidth    = 8;
    static const unsigned int AddressRange = 32;
    static const unsigned int AddressWidth = 20;
...
protected:
    ...
    Canny_accel_imgIndEe_memcore* Canny_accel_imgIndEe_memcore_U;
...
public:
    ...
    SC_CTOR(Canny_accel_imgIndEe) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        Canny_accel_imgIndEe_memcore_U = new Canny_accel_imgIndEe_memcore("Canny_accel_imgIndEe_memcore_U");
        Canny_accel_imgIndEe_memcore_U->clk      ( clk );
        Canny_accel_imgIndEe_memcore_U->reset    ( reset );
        Canny_accel_imgIndEe_memcore_U->ce0      ( i_ce0 );
        Canny_accel_imgIndEe_memcore_U->we0      ( i_we0 );
        Canny_accel_imgIndEe_memcore_U->address0 ( memcore_iaddr );
        Canny_accel_imgIndEe_memcore_U->d0       ( i_d0 );
        Canny_accel_imgIndEe_memcore_U->q0       ( i_q0 );

        Canny_accel_imgIndEe_memcore_U->ce1      ( t_ce0 );
        Canny_accel_imgIndEe_memcore_U->we1      ( t_we0 );
        Canny_accel_imgIndEe_memcore_U->address1 ( memcore_taddr );
        Canny_accel_imgIndEe_memcore_U->d1       ( t_d0 );
        Canny_accel_imgIndEe_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "Canny_accel_imgIndEe.trace");
            // ports
